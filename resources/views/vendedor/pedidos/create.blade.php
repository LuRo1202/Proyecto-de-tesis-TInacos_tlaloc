...
<div class="container-fluid px-4">
    ...
    <form method="POST" action="{{ route('vendedor.pedidos.store') }}" id="form-pedido" novalidate>
        @csrf
        <input type="hidden" name="sucursal_id" id="sucursal_id" value="{{ $sucursal->id }}">
        <input type="hidden" name="sucursal_nombre" id="sucursal_nombre" value="{{ $sucursal->nombre }}">
                    ...
                    <div class="col-12">
                        <div class="d-grid">
                            ...
                            <small class="text-muted mt-2 text-center d-block">
                                ...
                                Verificamos si entregamos en tu zona (radio de {{ $sucursal->radio_cobertura_km }}km de nuestras sucursales)
                            </small>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        @if(session('cobertura_verificada_vendedor'))
            ...
            coberturaVerificada = true;
            $('#btn-crear-pedido').prop('disabled', false);
            $('#sucursal-nombre').text(cobertura.sucursal_nombre);
            $('#sucursal-direccion').text(cobertura.sucursal_direccion);
            $('#distancia').text(cobertura.distancia);
            $('#cobertura-verificada-box').show();
